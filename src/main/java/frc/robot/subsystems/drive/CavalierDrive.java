...
public class CavalierDrive extends RobotDriveBase implements Sendable, AutoCloseable {
   ...
   */
  public CavalierDrive(CANSparkMax leftMotor, CANSparkMax rightMotor) {
  ...
  private void verify(CANSparkMax leftMotor, CANSparkMax rightMotor) {
  ...
  public void curvatureVelDrive(double xSpeed, double zRotation, boolean isQuickTurn) {
    ...
    double leftVoltageOutput = RobotMap.MAX_VOLTAGE * (leftMotorOutput * m_maxOutput);
    double rightVoltageOutput = RobotMap.MAX_VOLTAGE * (rightMotorOutput * m_maxOutput * m_rightSideInvertMultiplier);  

    double leftVel = (m_leftMotor.getEncoder().getVelocity() / RobotMap.DRIVE_RATIO) * RobotMap.WHEEL_DIAMETER * Math.PI / 60;
    double rightVel = (m_rightMotor.getEncoder().getVelocity() / RobotMap.DRIVE_RATIO) * RobotMap.WHEEL_DIAMETER * Math.PI / 60;
    ...
    if(leftVoltageOutput > RobotMap.MAX_VOLTAGE) {
        leftVoltageOutput = RobotMap.MAX_VOLTAGE;
    } else if (leftVoltageOutput < -RobotMap.MAX_VOLTAGE) {
        leftVoltageOutput = -RobotMap.MAX_VOLTAGE;
    }
    if(rightVoltageOutput > RobotMap.MAX_VOLTAGE) {
        rightVoltageOutput = RobotMap.MAX_VOLTAGE;
    } else if (rightVoltageOutput < -RobotMap.MAX_VOLTAGE) {
        rightVoltageOutput = -RobotMap.MAX_VOLTAGE;
    }

    //System.out.println("output " + rightVoltageOutput);

    m_leftMotor.setVoltage(leftVoltageOutput);
    m_rightMotor.setVoltage(rightVoltageOutput);
